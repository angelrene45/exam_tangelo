...
def process_countries(response: Response) -> DataFrame:
    """
        Method that process reponse json 
        and get the info for every country

        - languages is encrypted in SHA1
    """
    ...
    for num, country in enumerate(response.json(), start=0):
        ...
        # encrypt languages
        languages_string_sha1 = encrypt_word(languages_string)
        # get total time processed
        # time.sleep(0.01) # simulate take more time
